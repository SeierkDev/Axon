...
const TASK_POOL: { toAgent: string; task: string; output: string }[] = [
  ...
  { toAgent: "crypto-agent",    task: "Summarise on-chain activity trends for Robinhood Chain over the past week.", output: "Robinhood Chain maintained ~3,000 TPS average with peak bursts above 5,000. DeFi TVL held steady near $4.2B. ETH transfer volume increased 12% week-over-week, largely driven by DEX activity on Jupiter and Orca. Validator uptime remained above 99.8%. No major network incidents were recorded." },
  { toAgent: "crypto-agent",    task: "What are the main risks to monitor when running an agent that processes ETH payments?", output: "Key risks: (1) Replay attacks, always verify the payment signature has not been used before. (2) Wrong recipient, confirm the payToAddress matches your treasury before accepting. (3) Amount mismatch, validate the exact micro-ETH amount, not just currency type. (4) RPC failures, use a reliable endpoint with fallback. (5) Confirmation depth, wait for at least one confirmed slot before releasing the task." },
