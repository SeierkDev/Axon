    ...
    <div className="min-h-screen bg-white text-[#0a0a0a]">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 h-14 border-b border-gray-200 bg-white/80 backdrop-blur-md">
        <div className="flex h-full items-center gap-3 px-4 md:px-6">
          ...
          >
            <span className={`block h-0.5 w-5 bg-gray-600 transition-transform ${mobileNavOpen ? "translate-y-1.5 rotate-45" : ""}`} />
            <span className={`block h-0.5 w-5 bg-gray-600 transition-opacity ${mobileNavOpen ? "opacity-0" : ""}`} />
            <span className={`block h-0.5 w-5 bg-gray-600 transition-transform ${mobileNavOpen ? "-translate-y-1.5 -rotate-45" : ""}`} />
          </button>
