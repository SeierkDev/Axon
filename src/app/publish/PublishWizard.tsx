...
interface AgentForm {
  ...
  price: string;          // "0.0002 ETH" or empty
  provider: Provider;
...
export default function PublishWizard() {
  ...
  function validateForm(): boolean {
    ...
    if (form.price.trim()) {
      if (!/^\d+(?:\.\d{1,18})?\s*ETH$/i.test(form.price.trim())) {
        errors.price = "Format: 0.0002 ETH";
      }
    }
    ...
    <div>
      ...
      {step === "configure" && auth && (
        <div>
          ...
          <Field label="Price per task" hint="Leave blank for free. Format: 0.0002 ETH">
            <input
              value={form.price}
              onChange={(e) => setForm((f) => ({ ...f, price: e.target.value }))}
              placeholder="0.0002 ETH"
              className={inputCls}
            />
