          ...
          <div key={day.date} className="flex flex-col items-center gap-1 flex-1">
            ...
            <div className="w-full flex flex-col justify-end" style={{ height: 80 }}>
              {total > 0 ? (
                ...
                >
                  <div className="bg-gray-900 w-full" style={{ height: `${100 - failedPct}%` }} />
                  {failedPct > 0 && <div className="bg-gray-300 w-full" style={{ height: `${failedPct}%` }} />}
                </div>
              ) : (
                <div className="w-full rounded-sm bg-gray-100" style={{ height: 3 }} />
              )}
            </div>
      ...
      {agents.map((agent, i) => (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-gray-900 truncate group-hover:text-gray-700">{agent.name}</p>
            <p className="text-xs font-mono text-gray-400 truncate">{agent.agentId}</p>
          </div>
          <div className="text-right shrink-0">
            <p className="text-sm font-semibold text-gray-900">{agent.reputation.toFixed(1)}</p>
