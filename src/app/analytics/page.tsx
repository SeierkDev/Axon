      ...
      <main className="max-w-6xl mx-auto px-6 pt-32 pb-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          ...
          <div className="lg:col-span-2 p-5 rounded-xl border border-gray-200 bg-white">
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm font-semibold text-gray-900">Task Activity</p>
              <div className="flex items-center gap-4 text-xs text-gray-400">
                <span className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-sm bg-gray-900 inline-block" />Completed
                </span>
                <span className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-sm bg-gray-300 inline-block" />Failed
                </span>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="p-5 rounded-xl border border-gray-200 bg-white">
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm font-semibold text-gray-900">Top Agents</p>
              <Link href="/agents?sort=reputation" className="text-xs text-gray-400 hover:text-gray-900 transition-colors">
                View all →
              </Link>
            </div>
          ...
          <div className="p-5 rounded-xl border border-gray-200 bg-white">
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm font-semibold text-gray-900">Top Capabilities</p>
              <Link href="/capabilities" className="text-xs text-gray-400 hover:text-gray-900 transition-colors">View all →</Link>
            </div>
            {stats.topCapabilities.length === 0 ? (
              <p className="text-xs text-gray-400">No capabilities yet.</p>
            ) : (
                ...
                {stats.topCapabilities.map((cap, i) => {
                  ...
                  return (
                    <Link key={cap.capability} href={`/agents?capability=${encodeURIComponent(cap.capability)}`} className="block group"
                      ...
                      <div className="flex items-center justify-between mb-1">
                        <p className="text-sm text-gray-700 group-hover:text-gray-900 transition-colors font-mono">{cap.capability}</p>
                        <p className="text-xs text-gray-400">{cap.agentCount} agent{cap.agentCount !== 1 ? "s" : ""}</p>
                      </div>
                      <div className="w-full h-1 bg-gray-100 rounded-full overflow-hidden">
                        <div className="h-full bg-gray-800 rounded-full group-hover:bg-gray-900 transition-colors animate-grow-up"
