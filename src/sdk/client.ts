...
import { decodeRequirements, buildPaymentHeader } from "@/lib/x402";
...
export class AxonClient {
  ...
  // Full challenge-response identity verification.
  // options.sign receives a one-time challenge and must return a base64-encoded Ed25519 signature.
  ...
  // Calls a gateway provider endpoint synchronously and returns the upstream response.
  // For paid providers, pass paymentSignature (Axon-native) or use submitGatewayCallX402
  // to handle the x402 payment dance automatically.
  ...
  // Like callGatewayProvider but handles x402 payment automatically.
  async callGatewayProviderX402(
    ...
    // Step 1 — probe payment requirements
    ...
    if (!paidRes.ok) {
      throw this.apiErrorFromText(
        paidRes.status,
        "POST",
        `/api/gateway/${pathPart(providerId)}/call`,
        responseBody
      );
    }
  ...
  //
  // getX402Requirements returns the raw 402 response so callers can inspect payment terms.
  // submitTaskX402 handles the full protocol dance: discover → pay → submit.
  ...
  async getX402Requirements(agentId: string): Promise<X402Requirements | null> {
    ...
    if (res.status === 200) return null; // free agent
    if (res.status !== 402) {
    ...
    // A 402 with no header means a server-side bug — surface it rather than silently returning null
  ...
  async submitTaskX402(
    ...
    // Step 1 — probe for payment requirements using the shared decoder
    const probeRes = await fetch(`${this.baseUrl()}/api/agents/${pathPart(agentId)}/x402`, { method: "GET" });

    // Free agent — fall back to standard task submission
    ...
    // Step 2 — caller makes the on-chain payment
    const { signature, from } = await pay(requirements);

    // Step 3 — build X-Payment header using the network from the server's requirements
    // (never read process.env here — this may run in a browser)
    const network = requirements.accepts[0]?.network ?? "solana-mainnet";
    const paymentHeader = buildPaymentHeader(signature, from, network);
    ...
    const submitRes = await fetch(`${this.baseUrl()}/api/agents/${pathPart(agentId)}/x402`, {
      ...
      // body.from is intentionally omitted for paid tasks — the route uses paymentHeader.payload.from
