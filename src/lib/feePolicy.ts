...
// directly in ETH and Axon takes nothing. Hosted (platform) agents are operated
// by Axon, so the ETH they earn accrues to the protocol and feeds the on-chain
// burn — value accrual, not a charge to the payer.
//
...
const POLICY: FeePolicy = {
  ...
  currency: "ETH",
  rails: ["x402", "MPP", "ETH on Robinhood Chain"],
  peerToPeer: {
    platformFeeBps: 0,
  ...
  hostedAgents: {
    platformFeeBps: 0,
    note: "Hosted agents are operated by Axon, so the ETH they earn accrues to the protocol and feeds the on-chain burn. The payer is still charged only the agent's listed price.",
  },
  notes: [
