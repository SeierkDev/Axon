import { z } from "zod";
import { axonEndpoint, searchAgents, hireAgent, getProofScore, receiptUrl, parseUsdcPrice } from "./connector.js";
...
const safe = (fn) => async (agent, input) => {
    try {
...
export const searchAxonAgentsAction = {
    ...
    description: "Search the Axon marketplace for proven specialist agents by capability, ranked by verifiable Proof Score. Use this to find an agent to hire before calling AXON_HIRE_AGENT.",
    examples: [[
            {
    ...
    handler: safe(async (agent, input) => {
...
export const hireAxonAgentAction = {
    ...
    handler: safe(async (agent, input) => {
        ...
        if (input.maxPrice) {
            ...
            if (p == null)
                return { status: "error", message: 'maxPrice must be a USDC amount like "0.20 USDC"' };
            maxPriceUsdc = p;
        }
...
export const axonReceiptAction = {
    name: "AXON_RECEIPT",
    similes: ["get the axon receipt", "show the verifiable receipt for a task", "proof of a hire"],
    description: "Get the public, on-chain-verifiable receipt URL for a hired Axon task. Anyone can open it to see the parties, input/output hashes, settlement, and execution trace, and recompute the proof.",
    examples: [[
            {
...
export const verifyAxonProofScoreAction = {
    name: "AXON_VERIFY_PROOF_SCORE",
    similes: ["check an agent's proof score", "verify axon reputation", "how proven is this agent"],
    description: "Fetch an Axon agent's Proof Score and the public evidence behind it — reputation computed from its settled, on-chain receipts. Use to vet an agent before hiring.",
    examples: [[
            {
    ...
    handler: safe(async (agent, input) => {
        const base = axonEndpoint(agent);
        const score = (await getProofScore(base, input.agentId));
