...
export function axonEndpoint(agent) {
    const cfg = agent.config?.AXON_ENDPOINT ?? undefined;
...
export async function searchAgents(base, q) {
    ...
    if (!res.ok)
        throw new Error(`Axon agent search failed: HTTP ${res.status}`);
    const { agents } = (await res.json());
    return agents;
}
/** Probe an agent's price via x402. Returns null for a free-lane agent. */
async function getRequirement(base, agentId) {
    const res = await fetch(`${base}/api/agents/${encodeURIComponent(agentId)}/x402`);
    if (res.status === 200)
        return null; // free lane
    // A non-USDC (e.g. SOL) price returns 503 from the server; this plugin pays USDC only.
    if (res.status === 503)
        throw new Error(`Agent "${agentId}" is not priced in USDC — this plugin only pays USDC hires.`);
    if (res.status !== 402)
        throw new Error(`Axon price probe failed: HTTP ${res.status}`);
    const raw = res.headers.get("x-payment-required");
    if (!raw)
        throw new Error("Axon x402: missing X-Payment-Required header");
    let parsed;
    try {
    ...
    if (!opt)
        throw new Error("Axon x402: no payment option offered");
    return {
...
async function payFromWallet(agent, req, priorityFeeMicroLamports) {
    ...
    const tx = new Transaction().add(ComputeBudgetProgram.setComputeUnitLimit({ units: 60_000 }), ComputeBudgetProgram.setComputeUnitPrice({ microLamports: priorityFeeMicroLamports }), createAssociatedTokenAccountIdempotentInstruction(owner, toAta, req.payTo, req.mint), createTransferCheckedInstruction(fromAta, req.mint, toAta, owner, req.amount, req.decimals));
    ...
    if (conf.value.err)
        throw new Error(`payment failed on-chain: ${JSON.stringify(conf.value.err)}`);
    ...
    while (status !== "completed" && status !== "failed") {
        ...
        try {
            ...
            if (t.status === 401 || t.status === 403)
                break; // token rejected — stop, don't spin to the timeout
            if (t.ok) {
...
export async function getProofScore(base, agentId) {
    const res = await fetch(`${base}/api/agents/${encodeURIComponent(agentId)}/proof-score`);
    if (!res.ok)
        throw new Error(`Axon proof-score failed: HTTP ${res.status}`);
    return (await res.json());
}
export function receiptUrl(base, taskId) {
